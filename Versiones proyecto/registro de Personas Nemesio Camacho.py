...
if __name__ == "__main__":
    ...
    if not nombre_evento:
        logging.info("No se encontró evento  para hoy.")
        exit(0)
    ...
    #* Si la hora actual es menor que la hora de inicio del evento, esperar
    if ahora < hora_inicio:
        segundos_espera = (datetime.combine(datetime.today(), hora_inicio) - datetime.now()).total_seconds()
        logging.info("Esperando %d segundos hasta el inicio del evento '%s'...", segundos_espera, nombre_evento)
        time.sleep(segundos_espera)

    #! Generación del token de sesión
    #* Parámetros necesarios para la generación del token
